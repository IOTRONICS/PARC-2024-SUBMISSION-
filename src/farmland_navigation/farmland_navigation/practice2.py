...
class FarmNavigator(Node):
        ...
        while rclpy.ok():
            ...
            while self.obstacle_detected:
                ...
                if self.should_turn_left():
                    ...
                    self.turn(180, 2)
                    self.deviation_angle += 180
                else:
                    self.get_logger().info("Turning right to avoid peg")
                    self.turn_opposite(180, 2)
                    self.deviation_angle -= 180
                ...
                if not self.obstacle_detected:
                    ...
                    self.straight_movement()  # Move straight for a few seconds after avoidance
    ...
    def stop_movement(self):
        ...
        duration = 3  # duration of the stop in seconds
    ...
    def turn_opposite(self, angle_degrees, duration):
        self.get_logger().info("Executing turn opposite: " + str(angle_degrees) + " degrees for " + str(duration) + " seconds")
        move_cmd = Twist()
        move_cmd.linear.x = 0.0
        move_cmd.angular.z = -math.radians(angle_degrees) / duration
    ...
    def return_to_original_path(self):
        if self.deviation_angle != 0:
            self.get_logger().info("Returning to original path")
            self.turn(-self.deviation_angle, 2)
    ...
    def left_camera_callback(self, msg):
        self.process_camera_image(msg, 'left')

    def right_camera_callback(self, msg):
        self.process_camera_image(msg, 'right')
