...
class FarmNavigator(Node):
    ...
    def turn(self, angle_degrees, duration):
        self.get_logger().info(f"Executing turn: {angle_degrees} degrees for {duration} seconds")
        move_cmd = Twist()
        move_cmd.linear.x = 0.0
    ...
    def return_to_original_path(self):
        if self.deviation_angle != 0:
            ...
            turn_angle = self.deviation_angle if self.deviation_angle < 180 else self.deviation_angle - 360
            self.turn(turn_angle, 1)
            self.deviation_angle = 0
    ...
    def process_camera_image(self, msg, camera_position):
        ...
        if camera_position == 'center':
            ...
            if self.obstacle_detected:
                self.get_logger().info(f"Peg detected by center camera at distance {distance_to_bottom} on {peg_position} side")
            elif self.last_obstacle_state != 'cleared':
                self.get_logger().info("No peg detected by center camera")
                self.last_obstacle_state = 'cleared'
        
        cv2.imshow(f"{camera_position.capitalize()} Camera", cv_image)
        cv2.waitKey(1)
